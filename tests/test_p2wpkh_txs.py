...
class TestCreateP2wpkhTransaction(unittest.TestCase):
    def setUp(self):
        ...
        self.txout1 = TxOutput(int(0.0099 * COIN), self.p2wpkh_addr.to_script_pub_key())

        # P2WPKH to P2PKH
        self.txin_spend = TxInput("b3ca1c4cc778380d1e5376a5517445104e46e97176e40741508a3b07a6483ad3", 0)
        self.txin_spend_amount = int(0.0099 * COIN)
        self.txout2 = TxOutput(int(0.0098 * COIN), self.p2pkh_addr.to_script_pub_key())
        ...
        self.txin_spend_p2pkh_amount = int(0.01 * COIN)

        self.txin_spend_p2wpkh = TxInput("fff39047310fbf04bdd0e0bc75dde4267ae4d25219d8ad95e0ca1cee907a60da", 0)
        self.txin_spend_p2wpkh_amount = int(0.0095 * COIN)

        self.txout3 = TxOutput(int(0.0194 * COIN), self.p2pkh_addr.to_script_pub_key())

        # SIGHASH NONE type send
        self.txin1_signone = TxInput("fb4c338a00a75d73f9a6bd203ed4bd8884edeb111fac25a7946d5df6562f1942", 0)
        self.txin1_signone_amount = int(0.01 * COIN)

        self.txout1_signone = TxOutput(int(0.0080 * COIN), self.p2pkh_addr.to_script_pub_key())
        self.txout2_signone = TxOutput(int(0.0019 * COIN), self.p2pkh_addr.to_script_pub_key())

        # SIGHASH SINGLE type send
        self.txin1_sigsingle = TxInput("b04909d4b5239a56d676c1d9d722f325a86878c9aa535915aa0df97df47cedeb", 0)
        self.txin1_sigsingle_amount = int(0.0193 * COIN)

        self.txout1_sigsingle = TxOutput(int(0.01 * COIN), self.p2pkh_addr.to_script_pub_key())
        self.txout2_sigsingle = TxOutput(int(0.0092 * COIN), self.p2pkh_addr.to_script_pub_key())
        ...
        self.txin1_siganyonecanpay_all_amount = int(0.0018 * COIN)

        self.txin2_siganyonecanpay_all = TxInput("f4afddb77cd11a79bed059463085382c50d60c7f9e4075d8469cfe60040f68eb", 0)
        self.txin2_siganyonecanpay_all_amount = int(0.0018 * COIN)

        self.txout1_siganyonecanpay_all = TxOutput(int(0.0018 * COIN), self.p2pkh_addr.to_script_pub_key())
        self.txout2_siganyonecanpay_all = TxOutput(int(0.0017 * COIN), self.p2pkh_addr.to_script_pub_key())
        ...
        self.txin1_siganyonecanpay_none_amount = int(0.009 * COIN)

        self.txin2_siganyonecanpay_none = TxInput("ee5062d426677372e6de96e2eb47d572af5deaaef3ef225f3179dfa1ece3f4f5", 0)
        self.txin2_siganyonecanpay_none_amount = int(0.007 * COIN)

        self.txout1_siganyonecanpay_none = TxOutput(int(0.008 * COIN), self.p2pkh_addr.to_script_pub_key())
        self.txout2_siganyonecanpay_none = TxOutput(int(0.007 * COIN), self.p2pkh_addr.to_script_pub_key())
        ...
        self.txin1_siganyonecanpay_single_amount = int(0.01 * COIN)

        self.txout1_siganyonecanpay_single = TxOutput(int(0.005 * COIN), self.p2pkh_addr.to_script_pub_key())
        self.txout2_siganyonecanpay_single = TxOutput(int(0.0049 * COIN), self.p2pkh_addr.to_script_pub_key())
