...
class TestCreateP2wpkhTransaction(unittest.TestCase):
    def setUp(self):
        ...
        self.txout1 = TxOutput(int(COIN * 0.0097), self.p2wsh_addr.to_script_pub_key())

        # P2WSH to P2PKH
        self.txin_spend = TxInput("6233aca9f2d6165da2d7b4e35d73b039a22b53f58ce5af87dddee7682be937ea", 0)
        self.txin_spend_amount = int(COIN * 0.0097)
        self.txout2 = TxOutput(int(COIN * 0.0096), self.p2pkh_addr.to_script_pub_key())
        ...
        self.txin1_multiple_amount = int(COIN * 0.005)
        # P2WSH UTXO
        self.txin2_multiple = TxInput("65f4d69c91a8de54dc11096eaa315e84ef91a389d1d1c17a691b72095100a3a4", 0)
        self.txin2_multiple_amount = int(COIN * 0.0069)
        # P2WPKH UTXO
        self.txin3_multiple = TxInput("6c8fc6453a2a3039c2b5b55dcc59587e8b0afa52f92607385b5f4c7e84f38aa2", 0)
        self.txin3_multiple_amount = int(COIN * 0.0079)

        self.output1_multiple = TxOutput(int(COIN * 0.001), self.p2wsh_addr.to_script_pub_key())
        self.output2_multiple = TxOutput(int(COIN * 0.001), self.sk1.get_public_key().get_segwit_address().to_script_pub_key())
        self.output3_multiple = TxOutput(int(COIN * 0.0177), self.p2pkh_addr.to_script_pub_key())
