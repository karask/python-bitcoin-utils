...
class TestCreateP2shTransaction(unittest.TestCase):
    def setUp(self):
        ...
        self.p2pk_redeem_script = Script([self.p2pk_sk.get_public_key().to_hex(),
                                          'OP_CHECKSIG'])
        self.txout = TxOutput(Decimal(COIN * 0.09), self.p2pk_redeem_script.to_p2sh_script_pub_key() )
        ...
        self.txout2 = TxOutput(Decimal(COIN * 0.08), self.to_addr.to_script_pub_key() )
    ...
    def test_spend_p2sh_csv_p2pkh(self):
        ...
        txout = TxOutput(11 * COIN, self.another_addr.to_script_pub_key())
