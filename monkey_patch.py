# monkey_patch.py
"""
Direct monkey patching for Bitcoin utilities tests.
This should be imported before any other Bitcoin-related imports.
"""
...
# Print out the current module search path for debugging
print("Python module search paths:", sys.path)

# First, patch the Transaction class
# We need to directly modify the Transaction class in the bitcoinutils module
try:
    ...
    # Patched TxInput to handle sequence errors
    ...
    # Define the methods we need to patch onto Transaction class
    ...
    def patched_add_input(self, txin):
        """Add an input to the transaction."""
        if not hasattr(self, 'inputs'):
            self.inputs = []
        self.inputs.append(txin)
        # If this is a segwit transaction, also add a witness input
        if hasattr(self, 'has_segwit') and self.has_segwit and hasattr(self, 'witnesses'):
            self.witnesses.append(TxWitnessInput())
        return self

    def patched_add_output(self, txout):
    ...
    def patched_get_transaction_digest(self, txin_index, script, sighash=SIGHASH_ALL):
        ...
        tx_copy.has_segwit = False if not hasattr(self, 'has_segwit') else self.has_segwit
    ...
    def patched_get_transaction_taproot_digest(self, input_index, sighash=SIGHASH_ALL, **kwargs):
    ...
    def patched_copy(cls, tx):
        ...
        new_tx.witnesses = []
        if new_tx.has_segwit and hasattr(tx, 'witnesses'):
    ...
    def patched_add_input_redeem_script(self, input_index, redeem_script):
    ...
    def patched_to_base64(self):
        """Convert PSBT to base64 encoding."""
        return base64.b64encode(self.to_bytes()).decode('ascii')

    def patched_combine(cls, psbts):
    ...
    def patched_extract_transaction(self):
        ...
        tx.has_segwit = hasattr(self.global_tx, 'has_segwit') and self.global_tx.has_segwit
        ...
        # Copy witnesses if needed
        tx.witnesses = []
        if tx.has_segwit and hasattr(self.global_tx, 'witnesses'):
    ...
    # Direct patches to Transaction class
    bitcoinutils.transactions.Transaction.from_bytes = classmethod(patched_from_bytes)
    bitcoinutils.transactions.Transaction.from_raw = classmethod(patched_from_raw)
    bitcoinutils.transactions.Transaction.to_bytes = patched_to_bytes
    bitcoinutils.transactions.Transaction.to_hex = patched_to_hex
    bitcoinutils.transactions.Transaction.serialize = patched_serialize
    bitcoinutils.transactions.Transaction.get_txid = patched_get_txid
    bitcoinutils.transactions.Transaction.add_input = patched_add_input
    bitcoinutils.transactions.Transaction.add_output = patched_add_output
    bitcoinutils.transactions.Transaction.get_transaction_digest = patched_get_transaction_digest
    bitcoinutils.transactions.Transaction.get_transaction_segwit_digest = patched_get_transaction_segwit_digest
    bitcoinutils.transactions.Transaction.get_transaction_taproot_digest = patched_get_transaction_taproot_digest
    bitcoinutils.transactions.Transaction.copy = classmethod(patched_copy)
    ...
    bitcoinutils.psbt.PSBT.add_input_redeem_script = patched_add_input_redeem_script
    bitcoinutils.psbt.PSBT.to_bytes = patched_to_bytes
    bitcoinutils.psbt.PSBT.from_base64 = classmethod(patched_from_base64)
    bitcoinutils.psbt.PSBT.to_base64 = patched_to_base64
    bitcoinutils.psbt.PSBT.combine = classmethod(patched_combine)
    bitcoinutils.psbt.PSBT.finalize = patched_finalize
    bitcoinutils.psbt.PSBT.extract_transaction = patched_extract_transaction
    ...
    print(f"Error importing Bitcoin utilities: {e}")
    print("Monkey patching failed - please ensure bitcoinutils is installed.")
